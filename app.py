...
if user_role in ["Admin", "Superadmin"] and st.session_state.user:
    ...
    if url_input:
        try:
            ...
            response = requests.get(url_input, verify=False)  # Disable SSL verification
            soup = BeautifulSoup(response.text, "html.parser")
            website_text = soup.get_text()

            # Save website content as a text file with UTF-8 encoding
            website_file_path = os.path.join(UPLOAD_DIR, "website_content.txt")
            with open(website_file_path, "w", encoding="utf-8") as f:  # Specify UTF-8 encoding
                f.write(website_text)

            # Process website content with UTF-8 encoding
            with open(website_file_path, "r", encoding="utf-8") as f:  # Specify UTF-8 encoding
                text = f.read()
            ...
            # Add to FAISS index
            faiss_index.add(text_embedding.reshape(1, -1))
            pdf_metadata[len(pdf_metadata)] = {"file": "website_content.txt", "text": text}

            st.sidebar.success(f"✅ Website content scraped and uploaded successfully!")
        except Exception as e:
            st.sidebar.error(f"❌ Error scraping website: {e}")
